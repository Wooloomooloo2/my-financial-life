...
- [x] **P8b — the register scrolls its two most important columns off-screen — DONE (ADR-162).** `register_window.py` sets fixed pixel widths per column with `setStretchLastSection(False)`, so **Memo — empty on most rows — holds a wide fixed slot while Amount and Balance fall off the right edge** behind a horizontal scrollbar. A ledger whose money columns need scrolling to see. Memo should be the column that flexes; Amount and Balance should be pinned and never clipped.
- [x] **P8c — the chart palette is not the brand's — DONE (ADR-166), and it was not merely off-brand: it was inaccessible.** The old 12-colour list's comment claimed "tested at AA contrast"; it had not been. **violet-500 vs blue-600 measured ΔE 3.3 under protanopia** — the same colour to a red-blind reader — and four of the twelve sat below 3:1 on white. Replaced with **8 validated slots** (teal-led, fixed order, no cycling), light and dark separately validated. **Follow-ups below.**
  - [ ] **Fold to "Other" in the remaining charts.** Only Spending / Income Over Time fold their tail. The **treemap, sunburst, Sankey and Category & Payee can still exceed 8 series**, where `colour_for` wraps and a 9th slice repeats slot 1's teal — the exact bug ADR-166 fixed elsewhere. Each needs the same fold.
  - [ ] **Colour follows rank, not entity.** Groups are ordered by total, so a filter that removes the largest category **repaints the survivors**. The correct model is a stable category→slot map (plus a policy for >8 entities). Bigger than ADR-166; deliberately deferred. The identity is the teal; the stacked bars use saturated primaries (blue/green/orange/red/purple/cyan/pink/lime) that read as a library default and fight the chrome around them. Wants a calibrated categorical palette anchored on the teal, validated in **both** themes — not an eyeballed swap.
- [x] **P8d — empty states read as breakage — DONE (ADR-163).** _(The Budget card is deliberately unchanged: "£0.00 of £2,474.00 spent this month" is true and useful on a month that has just started, unlike an empty top-payees list.)_ On Home, *Top Payees · This Month* and *Top Categories · This Month* both say "No spending yet this month" and the Budget card shows "£0.00 of £2,474.00 budgeted this month" over an empty bar. Three dead cards down one column make a working app look broken. Fall back to the **last complete month**, explicitly labelled.
- [x] **P8e — a new report's title is literally "Untitled" — DONE (ADR-164).** It is the largest text on the screen; the report's actual identity ("Spending Over Time") is the small grey subtitle beneath it. Invert — lead with the type, make unsaved state a quiet badge.
- [x] **P8f — the sidebar mixes currencies silently — DONE (ADR-165), though the finding as written was wrong.** The `$` on `US Brokerage $17,035.62` *is* a marker; the sidebar was right for USD. Pulling the thread found worse: **sixteen** modules carried a private copy of the currency table, most returning **no symbol at all** for a currency they didn't know — so a **CHF/CAD/AUD** balance rendered as a bare `1,234.00` next to sterling, and four modules were missing **JPY** outright. All sixteen now delegate to `currency_symbol()` (ADR-159), whose docstring had claimed to be "the single definition" since the day it landed. A source-scan test stops the seventeenth. The dangling `REPORTS` header is hidden when empty.
...
- [x] **The register clips Amount and Balance — DONE (ADR-162).** Fixed widths overflowed the default window by 193 px (account register) / 243 px (all transactions), and since the money columns are *last*, all of it landed on them — while Memo, empty on most rows, held a fixed 280 px. Memo now stretches; the money keeps a fixed width. **Still open for the investment register**, which carries eleven columns whose non-Memo widths (~1,090 px) exceed a 967 px viewport on their own — it fits a maximised window and scrolls below that. Frozen money columns would fix it properly; deferred as a bigger call (ADR-162).
...
> ⚠️ **Overtaken by ADR-193 (2026-08-19): the app is free.** The whole licensing subsystem —
> offline keys, the 30-day trial, the launch nag, Enter License / Buy, the signer — has been
> **deleted from the code**. Everything below records the model as decided in June 2026 and is
> kept as history; the pricing rewrite (and whatever the site says instead) is the owner's call.
...
- [x] ~~**Licensing mechanism — OFFLINE license key (DONE 2026-06-21, ADR-079 amendment).**~~ **Removed 2026-08-19 (ADR-193).** Built: Qt-free verify-only `licensing.py` (Ed25519 over `cryptography`, already a dep), `license_service.py` orchestration, `version.py`, About box + Enter-License dialog + Help menu + launch nag, `QSettings`-persisted key/trial (app-level), `tools/license_tool.py` offline signer (private key gitignored) + `cli license-check`. Ships a dev public key to be swapped for production.
- [x] ~~**Trial — DONE 2026-06-21.**~~ **Removed 2026-08-19 (ADR-193).** 30-day full-feature trial, `QSettings` first-write-wins (can't be reset by reinstall); enforcement deliberately gentle (dismissible nag + title cue, not a hard lockout) per ADR-079. Tighten to a hard gate later via `LicenseStatus.unlocked` if wanted.
