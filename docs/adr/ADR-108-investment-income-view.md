...
- The public demo currently has no DRIP/coupon rows in-window (actions present: `Div` / `Buy` / `ShrsIn`), so the reinvest toggle is a no-op *on the demo*; the unit test exercises the reinvest/coupon paths directly. A demo refresh adding a DRIP payer + bond coupons would make the toggle and the bond's yield visible end-to-end (deferred — owner).
