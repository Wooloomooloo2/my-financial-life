...
## Amendment (2026-06-12b) — per-security IRR column

**Why.** The deferred per-security IRR from the amendment above. The per-security table already showed ROI (cost-based, timing-blind); an IRR column lets the owner see *money-weighted* performance for each holding, side by side — which can diverge sharply from ROI for recently-transferred-in positions.

**Change.** `compute_returns` now tracks the IRR inputs **per security** in parallel with the combined ones: `SecurityReturn` gains the same four fields as `ReturnsResult` (`cash_flows`, `opening_market_value`, `terminal_market_value`, `irr_fully_priced`). The replay's flow recording goes through a small `add_flow(sid, …)` that appends to both the combined list and a `cash_flows_by_sec[sid]`; `snapshot_mv` returns a per-security market-value dict (so the opening bookend is split by security); the breakdown loop computes each security's terminal value (cost fallback when unpriced) and an `irr_incomplete_sec[sid]` flag set by any transfer/bookend cost fallback. The window converts each security's flows/bookends to the display currency (per currency group, at each flow's own date) and runs `xirr` once per merged row. A new **"IRR / yr"** column (sortable, sign-coloured, `—` when undefined, trailing `*` when approximate) sits after Return %.

**Verified** on the live snapshot + synthetic: two synthetic securities returned their closed-form IRRs (a doubler over ~4y → +18.9% = 2^¼−1; a −20% position → −5.4%); the live MAX table populated per-row (SUSA +8.3% after its merge, TSBIX +4.1%, DIVO +14.9%). Two instructive live cases, both correct-by-design: **SCHD** reads ROI +40.3% but IRR −2.6% because it's a mostly **un-merged in-kind transfer-in** (594 sh on 2025-10-31) whose out-leg is under a separate record, so the transfer doesn't cancel — IRR measures return *since the shares arrived* (~flat over ~7mo) while ROI compares to the low carried cost; **PGINX** shows `>+999%*` (capped + flagged) where an unpriced transfer leg fell back to cost. Both resolve as the owner merges the remaining renamed-fund duplicates (the transfer legs then cancel and the IRR reflects full holding history).

**Still open.** Same renamed-fund-duplicate dependency as the per-account basis carry (ADR-052/053): per-security IRR for a transferred position is "since transfer" until its duplicate is merged. No further IRR work planned.
