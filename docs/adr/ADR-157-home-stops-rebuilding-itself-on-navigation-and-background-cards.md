# ADR-157 — Home stops rebuilding itself on navigation and on background-card arrival

**Date:** 2026-07-12
**Status:** Implemented
**Related:** ADR-153 (the activation-refresh guard + `data_generation` — this finishes the job it started). ADR-075 (Home as stack page 0). ADR-150 (heavy Home cards computed off-thread). ADR-149 (Home refresh defers widget destruction).

## Context

ADR-153 fixed the freeze when *closing a report* by guarding `RegisterWindow.changeEvent` — the activation path — with `HomeView.refresh_if_stale()`. It claimed that was the whole of the problem. It was not.
...
1,169ms of synchronous rebuild, including a **797ms UI freeze**. None of it went through the path ADR-153 guarded. Two separate callers were still rebuilding unconditionally:
...
`_show_home()` calls `refresh_if_stale()` — the same guard ADR-153 put on the activation path. Returning to Home on unchanged data is now free. A real edit still bumps `data_generation` and still redraws, so the ADR-075 guarantee is untouched.
...
- **Cache inside `gather_home_data`.** Would help every caller, but it is a pure function over the repo and the right place for the memo is the generation token that already exists (ADR-153), not a second caching layer with its own invalidation rules to get wrong.
...
- Together with ADR-153, **all three** paths that rebuild Home (activation, navigation, background-card arrival) are now guarded. That is believed to be all of them; `refresh()` remains unconditional for the callers that genuinely need it (first-run, market-data update, file swap), each of which follows a known write.
