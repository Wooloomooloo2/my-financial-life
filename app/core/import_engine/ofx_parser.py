# ===========================================================================
# app/core/import_engine/ofx_parser.py
#
...
def _normalise_transaction(txn) -> Optional[dict]:
    ...
    Returns None if the transaction cannot be normalised.
    """
    # Amount: OFX uses signed amounts — negative = debit, positive = credit
    ...
    # Date: dtposted is a datetime object, we want just the date
    ...
    if not fitid:
        # Fall back to a hash of date + amount if FITID is missing
        import hashlib
