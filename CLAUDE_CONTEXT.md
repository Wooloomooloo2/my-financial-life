...
**Last updated:** 2026-06-07 — most recent session shipped a stream of import / register polish then planned the next two arcs as ADRs: the cross-currency entry gap (ADR-035 amendment 2026-06-07 — partner-side amount collected in the dialog, wired through New Transaction / inline edit / bulk edit / Post Now), an ISO 4217 currency picker on the Account dialog (typeahead combo replaces the bare 3-letter QLineEdit), inline amount editing on the register (Repository.update_transaction_amount handles non-transfer, same-currency transfer-pair sync, cross-currency rate recompute), the Banktivity sign-over-Type rule (**ADR-038** — Type column becomes informational; sign is authoritative across Deposit / Withdrawal / Transfer; closes the Bedford House opening-balance inversion + the Ally mis-tagged Withdrawal case), and a $-symbol fix on the Banktivity amount parser. Then turned to planning: **ADR-039 (Proposed)** for the saved-reports arc (new `report` + `report_folder` tables with per-type JSON filters; sidebar restructured into Accounts + Reports sections separated by a visible rule; Spending Over Time as the round-1 persisted type; Save / Save As / Open / Edit / Delete vocabulary; Reports menu narrows to bare-open verbs); **ADR-040 (Accepted)** for statement reconciliation (per-account modal dialog from the per-account summary's reserved RECONCILE row; `statement` + `statement_txn` schema; resume / variance / adjustment-txn flow; Reconciled rows become import-immutable). Implementation of both arcs is the next session. The earlier 2026-06-06 work shipped the multi-currency foundation (ADR-035), the transfer matcher (ADR-036), and the bulk reconcile screen (ADR-037). New visible surfaces: **Manage → Currencies…** (API key, refresh now, manual rates, matcher tunables), inline + bulk transfer matching dialogs replacing the always-create-partner behaviour, and **Manage → Reconcile Transfers…** (Ctrl+Shift+R) for pairing many candidates across two accounts at once. New modules: `mfl_desktop/fx.py` (openexchangerates.org client + launch-refresh helpers) and `mfl_desktop/transfer_reconcile.py` (pure-Python scorer + greedy pairer shared by single-flow matcher and reconcile dialog). Earlier-in-day work also covered: visual baseline + paintEvent charts (ADR-026), Create Schedule From Transaction (ADR-027), Payee aliases round 1 (ADR-029 + ADR-012 amendment), Spending Over Time rollup levels (ADR-030), hierarchical category picker (ADR-031), Vehicle account type (ADR-032), Per-account summary screen + polish + drill-down (ADR-033 / ADR-034 + period-preset amendment).
...
│   │   ├── 0010_reports.sql         # ADR-039: report + report_folder tables (next session)
│   │   └── 0011_reconciliation.sql  # ADR-040: statement + statement_txn + txn.statement_id (next session)
...
- **Saved reports arc planned 2026-06-07 as ADR-039 (Proposed).** Schema: new `report` + `report_folder` tables; per-type JSON `filters_json` blob keyed on `type` enum (`spending_over_time` / `net_worth` / `income_expense` / `sankey`). Sidebar restructured Banktivity-style: Accounts on top + Reports section below a visible separator; both support folders via per-section folder tables. **Round 1** ships migration 0010, the sidebar restructure, Spending Over Time as the first persisted type with filter controls for categories / payees / accounts / date range / rollup level, and the Save / Save As / Open / Edit / Delete vocabulary. Reports menu (menu bar) narrows to "open the bare report" verbs; saved-with-state instances live in the sidebar. **Rounds 2-4**: Net Worth, Income & Expense + Sankey (Sankey needs its own rendering decision), polish (drag-to-folder, export). Each round gets its own per-type ADR. Per-type filter dataclasses live in new `mfl_desktop/reports/filters.py` with `to_json` / `from_json` / `migrate` helpers — JSON-shape changes within a type don't need DB migrations. Memory: see [[project-sidebar-banktivity-layout]].
...
- **Statement reconciliation per account — planned 2026-06-07 as ADR-040 (Accepted).** Modal dialog from the per-account summary's reserved RECONCILE row (single-round, no multi-round arc). New `statement` + `statement_txn` tables (migration 0011) plus a nullable `txn.statement_id` column; statement has `status` enum (`open` / `reconciled` / `reconciled_with_variance`) and `closing_variance_pence` + optional `adjustment_txn_id`. Resume support via `status='open'` — only one open statement per account. Variance handling: close with zero-variance → straight Reconcile; close with non-zero variance → confirm dialog offers "Add adjustment" (auto-create a txn at statement date, system-seeded *Adjustment* category, sign closes the gap) or "Close with variance" (statement stamped `reconciled_with_variance`). Reopen reverts every linked txn's status to `Cleared` (the rare Pending/Uncleared-at-reconcile-time case is acceptable to lose). Reconciled rows become **import-immutable** — the import service's potential-match path skips them; inline edits on Reconciled rows pop a "this row is reconciled to statement X — change anyway?" confirm. Transfer pairs reconcile independently per side (the partner sits on a different account with its own statement date). Cross-currency accounts reconcile in their native currency — no conversion. Menu: **Account → Reconcile…** (Ctrl+Alt+R, avoiding the Ctrl+B Budget and Ctrl+Shift+R Reconcile-Transfers conflicts). Past-statement history list on the per-account summary screen; statement rows clickable for read-only view or "Reopen for editing".
